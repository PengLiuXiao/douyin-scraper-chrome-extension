...
function syncDOMToCache() {
    $('a').each(function() {
        ...
        if (match) {
            const id = match[1];
            if (!interceptedAwemes.has(id)) {
                const card = $(this).closest('li, [data-e2e*="card"], [data-e2e*="item"], .y2I5mU7s, div[class*="card"]');
                let title = '';
                let author = '';
                let likes = '';
                
                if (card.length) {
                    const titleEl = card.find('[data-e2e="search-card-video-title"], p, span[class*="desc"], span[class*="title"]');
                    if (titleEl.length) {
                        title = $(titleEl[0]).text().trim();
                    }
                    const authorEl = card.find('a[href*="/user/"], [class*="author"], [class*="user"]');
                    if (authorEl.length) {
                        author = $(authorEl[0]).text().trim();
                    }
                    const likeEl = card.find('[class*="like"], [class*="digg"], [data-e2e*="like"]');
                    if (likeEl.length) {
                        likes = $(likeEl[0]).text().trim();
                    }
                }
